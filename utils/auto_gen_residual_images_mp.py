...
def process_one_seq(config, seq, i):
    ...
    for frame_idx in tqdm(range(len(scan_paths))):
        ...
        diff_image = np.full((range_image_params['height'], range_image_params['width']), 0, dtype=np.float32)  # [H,W] range (0 is no data)
...
if __name__ == '__main__':
    ...
    split_seq_list = [all_seq_list[i:i + (len(all_seq_list) // seq_split_part)] for i in range(0, len(all_seq_list), len(all_seq_list) // seq_split_part)]
    split_residual_list = [residual_list[i:i + (len(residual_list) // residual_split_part)] for i in range(0, len(residual_list), len(residual_list) // residual_split_part)]
        ...
        for seq_id in range(len(split_seq_list)):
            ...
            for seq in split_seq_list[seq_id]:
                for i in split_residual_list[residual_id]:
                    config['num_last_n'] = i
                    config['scan_folder'] = os.path.join(dataset, f"sequences/{'%02d' % seq}/velodyne")
                    config['pose_file'] = os.path.join(dataset, f"sequences/{'%02d' % seq}/poses.txt")
                    config['calib_file'] = os.path.join(dataset, f"sequences/{'%02d' % seq}/calib.txt")
                    config['residual_image_folder'] = os.path.join(dataset, f"sequences/{'%02d' % seq}/residual_images_{i}")
                    config['visualization_folder'] = os.path.join(dataset, f"sequences/{'%02d' % seq}/visualization_{i}")

                    if os.path.exists(config['residual_image_folder']):
                        if len(os.listdir(config['residual_image_folder'])) == len(os.listdir(config['scan_folder'])):
                            print(f"seq {seq} residual {i} has been created!")
                            continue

                    process = Process(target=process_one_seq, args=(config, seq, i))  # 指定进程要执行的函数和参数
